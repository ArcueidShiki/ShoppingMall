-- 数据库：mall
use mall;
-- 相册表：创建数据表以下是注释是对原来的sql文件部分修改的说明
-- 所有表的时间均自动生成，无需手动设置:
...
-- 图片表：创建数据表 轮播图表整合到 picture表 和 album相册表
...
-- 属性模版表：为属性模版名称字段添加索引
create index idx_attribute_template_name on attribute_template (name);

-- 类别与属性模版关联表：创建数据表
drop table if exists category_attribute_template;
...
(
    ...
    price                  decimal(10, 2)   default null comment '价格（显示在列表中）',
    old_price              decimal(10, 2)   default null comment '原价（显示在列表中）',
    ...
    gmt_check              timestamp        default null on update current_timestamp comment '审核通过时间（冗余）',
    gmt_create             timestamp not null default current_timestamp comment '数据创建时间',
    gmt_modified           timestamp not null default current_timestamp on update current_timestamp comment '数据最后修改时间',
...
-- 品牌表：插入测试数据
insert into brand (name, pinyin, description, keywords, enable)
...
-- 类别表：插入测试数据
insert into category (name, parent_id, depth, is_parent, keywords, enable, is_display)
...
-- 关联测试查询：各品牌有哪些类别的产品
select brand_category.id, brand.name, category.name
from brand_category
         left join brand
                   on brand_category.brand_id = brand.id
         left join category
                   on brand_category.category_id = category.id
order by brand.pinyin;

-- 属性表：插入测试数据
insert into attribute (name, description, type, input_type, value_list, unit, is_allow_customize)
...
-- 属性模版表：插入测试数据
insert into attribute_template (name, pinyin, keywords)
...
-- 图片表：插入测试数据
insert into picture (album_id, url, description, width, height)
...
-- SPU表：插入测试数据
insert into spu (id, name, type_number, title, description, price, stock, stock_threshold, unit, brand_id,
...
insert into sku (id, spu_id, title, attribute_template_id, specifications, price, stock, stock_threshold)
...
insert into sku_specification (sku_id, attribute_id, attribute_name, attribute_value, unit)
...
desc brand_category;
desc attribute;
desc attribute_template;
desc spu;
desc spu_detail;
desc sku;
desc sku_specification;
